...
record MultiAnchorDescriptor(Chain chain, StartPlan startPlan, RejectPlan rejectPlan) {
  ...
  Anchor[] anchors() {
    Anchor[] result = new Anchor[chain.segments().length];
    for (int i = 0; i < chain.segments().length; i++) {
      result[i] = chain.segments()[i].anchor();
    }
    return result;
  }

  Gap[] gaps() {
    Gap[] result = new Gap[chain.segments().length + 1];
    for (int i = 0; i < chain.segments().length; i++) {
      result[i] = chain.segments()[i].gap();
    }
    result[chain.segments().length] = chain.trailingGap();
    return result;
  }
