...
final class IncubatorVectorScanProvider implements VectorScanProvider {
  ...
  @Override
  public int indexOfBytePair(byte[] bytes, int offset, int length, byte b0, byte b1, int start) {
    return ByteVectorScan.indexOfBytePair(bytes, offset, length, b0, b1, start);
  }

  @Override
  public int indexOfByteTriple(
      byte[] bytes, int offset, int length, byte b0, byte b1, byte b2, int start) {
    return ByteVectorScan.indexOfByteTriple(bytes, offset, length, b0, b1, b2, start);
  }
